...
if token_check.status_code != 200:
    ...
    print(f"  Fix: Create a CLASSIC token with 'repo' (full) scope.")
    exit(1)
scopes = token_check.headers.get("X-OAuth-Scopes", "")
print(f"  [OK]  Token valid. Scopes: '{scopes}'")
if "repo" not in scopes:
    print(f"  [ERROR] Token missing 'repo' scope! Current scopes: {scopes}")
...
def repo_is_empty_check():
    r = requests.get(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}/git/refs", headers=HEADERS)
    if r.status_code == 200 and r.json() == []:
        return True
    r2 = requests.get(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}/git/ref/heads/main", headers=HEADERS)
    if r2.status_code == 404:
        return True
    return False

def delete_repo():
    r = requests.delete(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}", headers=HEADERS)
    return r.status_code == 204

if repo_exists():
    if repo_is_empty_check():
        print(f"  [Fix] Repo is empty/broken — deleting for clean start...")
        if delete_repo():
            print(f"  [OK]  Deleted. Recreating...\n")
            time.sleep(2)
        else:
            print(f"  [ERROR] Could not delete repo. Check token has 'delete_repo' scope.")
            exit(1)
    else:
        print(f"  [Info] Repo exists with commits — will update files.\n")
...
if res.status_code == 201:
    ...
    print(f"  [OK]  Repo exists with commits — updating files.")
else:
    print(f"  [ERROR] {res.status_code}: {res.json().get('message')}")
    exit(1)
time.sleep(2)

repo_info      = requests.get(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}", headers=HEADERS).json()
default_branch = repo_info.get("default_branch", "main")
refs_check     = requests.get(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}/git/refs", headers=HEADERS)
branch_check   = requests.get(f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}/git/ref/heads/{default_branch}", headers=HEADERS)
is_empty       = (branch_check.status_code == 404) or (refs_check.status_code == 200 and refs_check.json() == [])
print(f"  [Info] Empty: {is_empty} | Branch: {default_branch}\n")

def push_initial_commit(files):
    ...
    for filepath, content in files.items():
        ...
        if r.status_code != 201:
            ...
            return False
        tree_items.append({"path": filepath, "mode": "100644", "type": "blob", "sha": r.json()["sha"]})
    ...
    r = requests.post(f"{base}/commits", headers=HEADERS, json={
        "message": f"Initial commit: {topic[:60]}", "tree": tree_sha, "parents": [],
    })
    if r.status_code != 201:
    ...
    r = requests.post(f"{base}/refs", headers=HEADERS, json={
        "ref": f"refs/heads/{default_branch}", "sha": commit_sha,
    })
...
def push_file(filepath, content, commit_msg):
    ...
    body = {"message": commit_msg, "content": base64.b64encode(content.encode("utf-8")).decode("utf-8")}
...
all_files = {
    "src/main.py": code, "README.md": readme,
