...
====================================================
  ...
  Runs at 6:00 PM Pakistan Time (1:00 PM UTC)
...
# ── Force delete repo if exists, with confirmation ──────────
print(f"Checking repo '{repo_name}'...")
check = requests.get(
    f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}",
    headers=HEADERS
)
print(f"  [Info] Repo check status: {check.status_code}")

if check.status_code == 200:
    print(f"  [Fix] Repo exists — deleting...")
    d = requests.delete(
        f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}",
        headers=HEADERS
    )
    print(f"  [Info] Delete status: {d.status_code}")
    if d.status_code == 204:
        print(f"  [OK]  Deleted successfully.")
    else:
        print(f"  [ERROR] Delete failed: {d.status_code} — {d.text}")
        exit(1)
    # Wait for GitHub to fully process deletion
    for i in range(1, 7):
        time.sleep(5)
        verify = requests.get(
            f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}",
        ...
        print(f"  [Wait] Confirming deletion... attempt {i}/6 — status: {verify.status_code}")
        if verify.status_code == 404:
            print(f"  [OK]  Repo fully deleted.")
            break
    else:
        print(f"  [ERROR] Repo still exists after 30s. Aborting.")
        exit(1)
elif check.status_code == 404:
    print(f"  [OK]  Repo does not exist — creating fresh.")
else:
    print(f"  [WARN] Unexpected status {check.status_code} — attempting create anyway.")
...
print(f"  [Info] Create status: {res.status_code}")
if res.status_code == 201:
    print(f"  [OK]  Repo created!")
else:
    print(f"  [ERROR] {res.status_code}: {res.json()}")
    exit(1)
# Poll until branch is ready (auto_init takes variable time on GitHub)
print("  Waiting for branch to be ready...")
for i in range(1, 13):
    time.sleep(5)
    branch = requests.get(
        f"https://api.github.com/repos/{GITHUB_USER}/{repo_name}/branches/main",
        headers=HEADERS
    )
    print(f"  [Wait] Branch check {i}/12 — status: {branch.status_code}")
    if branch.status_code == 200:
        print(f"  [OK]  Branch is ready!")
        break
else:
    print(f"  [ERROR] Branch not ready after 60s. Aborting.")
    exit(1)
